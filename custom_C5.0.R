### Custom C5.0 trainControl

customC50 <- list(label = "C5.0", 
                  library = c("C50", "plyr"),
                  loop = function(grid) {     
                    loop <- ddply(grid, c("model", "winnow","minCases"),
                  ...
                  type = "Classification",
                  parameters = data.frame(parameter = c('trials', 'model', 'winnow', 'minCases'),
                                          class = c("numeric", "character", "logical", "numeric"),
                                          label = c('# Boosting Iterations', 'Model Type', 'Winnow', 'MinCases')),
                  grid = function(x, y, len = NULL, search = "grid") {
                    if(search == "grid") {
                      c5seq <- if(len == 1)  1 else  c(1, 10*((2:min(len, 11)) - 1))
                      c5seq2 <- if(len == 1)  2 else  c(2, 10*((2:min(len, 11)) - 1))
                      out <- expand.grid(trials = c5seq, model = c("tree", "rules"), winnow = c(TRUE, FALSE),minCases=c5seq2)
                    } else {
                      out <- data.frame(trials = sample(1:100, replace = TRUE, size = len),
                                        ...
                                        minCases = sample(1:50,replace = TRUE, size = len))
                    }
                    out
                  },
                  fit = function(x, y, wts, param, lev, last, classProbs, ...) { 
                    ...
                    {                           
                      ...
                      theDots$control$minCases <- param$minCases
                    } else theDots$control <- C5.0Control(winnow = param$winnow, minCases = param$minCases)
                  ...
                  sort = function(x) {
                    ...
                    x[order(x$trials, x$model, !x$winnow, x$minCases),]
                  },
                  trim = function(x) {
                    x$boostResults <- NULL
                    x$size <- NULL
